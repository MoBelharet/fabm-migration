...
type, extends(type_base_model), public :: type_migration_move
...
subroutine move_initialize(self, configunit)
    ...
    class (type_migration_move), intent(inout), target :: self
...
subroutine move_do(self, _ARGUMENTS_DO_)
    class (type_migration_move), intent(in) :: self
    ...
    _LOOP_BEGIN_
      ...
      ! write(*,*)'integral = ', integral
       ...
       !write(*,*)par,mortality_switch,local_loss,local
        ...
        term1 =  (-local  +  distributed * integral/thickness ) /self%tstep 
        term2 =  local * (1.0_rk - mortality_switch) * self%m * xstep
       ...
       _ADD_SOURCE_(self%id_target, term1 + term2)
